...
export function keypressListen() {
	...
	process.stdin.on('data', async (key) => {
		...
		switch (key) {
			...
			case 'n': // New menu
				isPrompting = true;
				await handleCreateNew(() => { isPrompting = false; });
				break;
		}
	});
}
...
async function handleCreateNew(doneCb) {
	if (!project.isRunning) {
		log('warn', 'Build process paused. Press [p] to resume if needed. Continuing creation.');
	}
	process.stdout.write('\nCreate [b]lock, [p]attern, [s]tyle? ');
	const typeKey = await new Promise(resolve => {
		const listener = (data) => {
	...
	if (!['b','p','s'].includes(typeKey)) {
		log('warn', 'Invalid selection.');
		if (doneCb) doneCb();
		return;
	}
	if (typeKey === 'b') {
		const name = await prompt('Block name: ');
		if (!name) { log('warn', 'No name provided.'); if (doneCb) doneCb(); return; }
		...
		const name = await prompt('Pattern name: ');
		if (!name) { log('warn', 'No name provided.'); if (doneCb) doneCb(); return; }
		const slug = utils.slugify(name);
		...
		const name = await prompt('Style variation name: ');
		if (!name) {
