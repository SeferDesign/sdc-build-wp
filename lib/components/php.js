	...
	async checkSyntax(entry) {
		try {
			...
			const detail = [error.stderr, error.stdout, error.message]
				.filter(Boolean)
				.map(value => value.replace?.(this.project.path, '') ?? value)
				.find(Boolean);
			if (detail) {
	...
	async build(entry, options) {
		...
		try {
			...
			const filterLines = str => str
				...
				: str;
			const filterPhpStartupNoise = str => str
				? str.split('\n').filter(line =>
					!line.includes('PHP Warning:  PHP Startup:') &&
					!line.includes('Unable to initialize module') &&
					!line.includes('Failed loading') &&
					!line.includes('Xdebug requires Zend Engine API version') &&
					!line.includes('Module compiled with module API=') &&
					!line.includes('PHP    compiled with module API=') &&
					!line.includes('These options need to match') &&
					!line.includes('in Unknown on line 0')
				).join('\n').trim()
				: str;
			const filteredStderr = filterLines(error.stderr);
			const filteredStdout = filterLines(error.stdout);
			const fallbackDetail = filterLines(error.message);
			const startupNoiseFreeStderr = filterPhpStartupNoise(filteredStderr);
			const startupNoiseFreeStdout = filterPhpStartupNoise(filteredStdout);
			if (filteredStderr && error.stderr.includes('No fixable errors were found')) {
				// No fixable errors were found
			} else if (
				(startupNoiseFreeStdout && (error.stdout.startsWith('ERROR:') || error.stdout.includes('FAILED TO FIX'))) ||
				(startupNoiseFreeStderr && !error.stderr.trim().startsWith('Time:'))
			) {
				const detail = startupNoiseFreeStdout || startupNoiseFreeStderr || filteredStdout || filteredStderr || fallbackDetail;
				this.log(null, detail);
				this.log('error', `Failed linting ${entryLabel.replace(this.project.path, '')} - See above error.`);
				return false;
			} else if (fallbackDetail) {
				this.log(null, fallbackDetail);
				this.log('error', `Failed linting ${entryLabel.replace(this.project.path, '')} - See above error.`);
				return false;
			}
		}
