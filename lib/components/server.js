...
export default class ServerComponent extends BaseComponent {
	...
	serve(watch = false) {
		...
		this.server.init(bsOptions, (err, bs) => {
			this.usedOptions = bs.options.toJS();
			if (err) {
	...
	setupSocketHandlers(serverComponent) {
		this.server.sockets.on('connection', (socket) => {
			...
			socket.on('sdc:redirectAdminFromPort', (data) => {
				this.log('info', `Port redirecting (${data.port}) from ${data.from} to ${data.to}`);
			});
		});
	}

	logURLs() {
		let localURL = this.usedOptions.urls.local;
